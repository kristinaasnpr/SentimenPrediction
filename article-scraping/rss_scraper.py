...
db = firestore.client()
collection_name = 'data_artikel'  # Ganti dengan nama koleksi yang Anda inginkan
...
# Define columns
COL_NO = 'No'
COL_JUDUL = 'Judul Artikel'
COL_ISI = 'Isi Artikel'
COL_LINK = 'Link'
COL_TANGGAL = 'Tanggal Terbit'
COL_SYMBOL = 'symbol'
COL_LABEL = 'Label'
...
def remove_img_src(text):
    """Remove image tags from the article text."""
...
def read_existing_data():
    """Read existing article titles from Firestore within the last 14 days."""
    existing_titles = set()
    try:
        ...
        for doc in docs:
            ...
            published_date = article_data.get(COL_TANGGAL)
            if published_date:
                published_date = datetime.datetime.strptime(published_date, "%Y-%m-%d")
                if published_date >= fourteen_days_ago:
                    existing_titles.add(article_data[COL_JUDUL])  # Ganti dengan nama kolom yang sesuai
    except Exception as e:
...
def find_symbols(content):
    """Find symbols based on keywords in the article content."""
    found_symbols = []
...
def parse_date_time(date_string):
    """Parse the date string and return only the date in 'YYYY-MM-DD' format."""
    if not date_string:
        return None
    try:
        parsed_date = parser.parse(date_string)
        return parsed_date.strftime('%Y-%m-%d')  # Hanya mengembalikan tanggal
    except Exception as e:
...
def scrape_rss_feeds():
    ...
    for feed_url in rss_feeds:
        ...
        for entry in feed.entries:
            ...
            formatted_date = parse_date_time(published)  # Hanya ambil tanggal
